    ...
    return (
        <>
            {socialPlatforms.map((item, index) => (
                <tr key={item.key} className={`block sm:table-row border-b ${index === socialPlatforms.length - 1 ? 'border-[#c3c4c7] sm:border-transparent' : 'border-[#c3c4c7] sm:border-[#f0f0f1]'}`}>
