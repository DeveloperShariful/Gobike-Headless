...
// 🛑 FIX: Added Warranty specific variables
const AVAILABLE_VARIABLES = [
    ...
    { key: "{description}", desc: "Warranty claim description" },
    { key: "{media_urls}", desc: "Customer uploaded video/image links" },
    { key: "{shop_purchased}", desc: "Name of the shop purchased from" },
];
...
export default function TemplateEditModal({ template, open, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-center items-center bg-black/50 backdrop-blur-sm p-2 sm:p-4">
      <div className="bg-[#f0f0f1] w-full max-w-[1000px] h-[95vh] sm:max-h-[90vh] flex flex-col rounded-sm shadow-xl overflow-hidden border border-[#c3c4c7]">
        
        <div className="bg-white px-4 sm:px-6 py-4 border-b border-[#c3c4c7] flex justify-between items-center shrink-0">
            <div>
                <h2 className="text-[16px] sm:text-[18px] font-semibold text-[#1d2327] m-0 pr-2">Edit Template: {template.name}</h2>
                <span className="text-[11px] sm:text-[12px] text-[#646970]">Trigger Event: {template.triggerEvent}</span>
            </div>
            <button onClick={onClose} className="text-[#646970] hover:text-red-500 text-[24px] leading-none px-2 shrink-0">&times;</button>
        </div>
        
        <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
            
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-white custom-scrollbar order-2 lg:order-1">
                <form id="template-form" onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                    <input type="hidden" name="id" value={template.id} />
                    
                    <div className="flex items-center gap-3 bg-[#f0f0f1] p-3 border border-[#c3c4c7] rounded-sm">
                        <input type="checkbox" name="isEnabled" id="isEnabled" defaultChecked={template.isEnabled} className="w-4 h-4 rounded-[3px] border-[#8c8f94] text-[#2271b1] focus:ring-[#2271b1] shrink-0"/>
                        <label htmlFor="isEnabled" className="text-[13px] sm:text-[14px] font-semibold text-[#1d2327] cursor-pointer leading-tight">Enable this automated email</label>
                    </div>
                    ...
                    <div className="flex flex-col flex-1 h-[400px] lg:h-[350px]">
                        <label className="block text-[13px] font-bold text-[#3c434a] mb-2 shrink-0">Email Body Content</label>
                        <div className="flex-1 border border-[#8c8f94] rounded-sm overflow-hidden min-h-[300px]">
            ...
            <div className="w-full lg:w-64 bg-[#f0f0f1] border-b lg:border-b-0 lg:border-l border-[#c3c4c7] overflow-y-auto p-4 shrink-0 order-1 lg:order-2 max-h-[200px] lg:max-h-full">
                <h4 className="text-[12px] font-bold text-[#646970] uppercase mb-3 sticky top-0 bg-[#f0f0f1] pb-1 z-10">Dynamic Variables</h4>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-1 gap-2">
                    {AVAILABLE_VARIABLES.map((v) => (
                        ...
                        >
                            <code className="text-[11px] font-bold text-[#2271b1] block break-all">{v.key}</code>
                            <p className="text-[10px] sm:text-[11px] text-[#646970] m-0 leading-tight mt-1">{v.desc}</p>
                        </div>
                    ))}
                </div>
            </div>
