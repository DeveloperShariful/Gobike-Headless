...
export default function UserFormClient({ initialData }: { initialData: any }) {
  ...
  const secondaryBtnClass = "bg-[#f6f7f7] border border-[#8c8f94] text-[#3c434a] px-3 py-1 text-[13px] rounded-[3px] hover:bg-[#f0f0f1] transition-colors cursor-pointer";
      ...
      <table className={tableClass}>
        <tbody className="block sm:table-row-group">
          ...
          <tr className="border-b border-[#c3c4c7] sm:border-[#f0f0f1] block sm:table-row">
            <th className={thClass}>Email <span className="text-red-500">*</span></th>
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2">
                    <input 
                      ...
                      className={`${inputClass} !w-[250px] font-mono`} 
                    />
                  ...
                  <span className={descClass}>
                    {isEditing ? "If you don't want to change the password, just click Cancel." : "You can edit the password or copy this one."}
                  </span>
                </div>
              )}
            </td>
