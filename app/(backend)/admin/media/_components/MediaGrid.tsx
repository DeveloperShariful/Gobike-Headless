      ...
      {filteredMedia.map((file, index) => {
        ...
        return (
          ...
          >
            ...
            {file.type === 'VIDEO' ? (
              <div className="w-full h-full flex flex-col items-center justify-center bg-[#1d2327] text-white p-2">
                <IoVideocamOutline size={32} className="text-gray-400 mb-2" />
                <span className="text-[10px] truncate w-full text-center px-1 text-gray-300">{file.filename}</span>
              </div>
            ) : file.type === 'DOCUMENT' ? (
