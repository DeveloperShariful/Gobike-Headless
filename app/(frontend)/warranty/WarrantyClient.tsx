...
import { submitWarrantyClaim } from '@/app/actions/frontend/warranty-action';
...
// WooCommerce checkout-এর মতো Address Form type
type AddressForm = {
  firstName: string;
  lastName: string;
  address1: string;
...
// অস্ট্রেলিয়ার সব State/Territory
const AU_STATES = [
...
export default function WarrantyClient() {
  ...
  // Address form state — শুধু non-GoBike shop এর জন্য
  ...
  // GoBike Online shop সিলেক্ট করা আছে কিনা
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Non-GoBike shop হলে address validation
    if (!isGoBikeOnline) {
      if (!addressForm.firstName || !addressForm.lastName || !addressForm.address1 || !addressForm.city || !addressForm.postcode || !addressForm.phone) {
    ...
    try {
      ...
      const result = await submitWarrantyClaim({
        ...
        // Non-GoBike shop হলে manual address পাঠাবো
        manualAddress: !isGoBikeOnline ? addressForm : undefined,
      });

      if (!result.success) throw new Error(result.message);

      setSuccessMessage(
        'Your warranty claim has been submitted successfully! Our technical team will review your video and contact you shortly.'
      );

      setFormData({ name: '', orderNumber: '', shopPurchased: GOBIKE_ONLINE_VALUE, email: '', description: '' });
      setAddressForm({ firstName: '', lastName: '', address1: '', address2: '', city: '', state: 'NSW', postcode: '', country: 'AU', phone: '' });
      setUploadedMedia([]);
      setUploadProgress(0);
    } catch (error: any) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-5 space-y-8">
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
              ...
              <div className="space-y-6">
                {[
                  { step: 1, title: 'Submit Your Details', desc: 'Provide your order info and a detailed description of the problem.' },
                  { step: 2, title: 'Upload a Clear Video', desc: 'Record a short video showing the issue so our mechanics can diagnose it accurately.' },
                  { step: 3, title: 'Fast Resolution', desc: 'Once approved, we will immediately dispatch the required replacement parts.' },
                ].map(({ step, title, desc }) => (
          ...
          <div className="lg:col-span-7">
            <div className="bg-white p-8 sm:p-10 rounded-[2rem] shadow-xl border border-gray-100 relative">
              <div className="absolute top-0 right-0 w-32 h-32 bg-blue-50 rounded-bl-[100px] rounded-tr-[2rem] -z-10" />

              <h2 className="text-2xl font-bold text-gray-900 mb-6">Submit Claim Form</h2>

              {successMessage && (
                <div className="mb-8 p-5 bg-green-50 border-l-4 border-green-500 rounded-r-xl text-green-800 font-medium shadow-sm">
                  ✓ {successMessage}
                </div>
              )}
              {errorMessage && (
                <div className="mb-8 p-5 bg-red-50 border-l-4 border-red-500 rounded-r-xl text-red-800 font-medium shadow-sm">
                  ✕ {errorMessage}
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-5 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                ...
                <div>
                  ...
                  >
                    <option value={GOBIKE_ONLINE_VALUE}>GoBike Australia (Online)</option>
                ...
                {/* ── Order Number: GoBike Online হলে Required, নাহলে Optional ── */}
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">
                    Order Number {isGoBikeOnline ? '*' : <span className="text-gray-400 font-normal">(Optional)</span>}
                  </label>
                  ...
                  {isGoBikeOnline && (
                    <div className="bg-blue-50 border border-blue-100 p-3 rounded-xl mb-3">
                      <p className="text-[13px] text-blue-800 font-medium flex items-center gap-2">
                        ...
                        <span>Use the exact email from your GoBike purchase. We will auto-verify your order!</span>
                      </p>
                    </div>
                  )}
                  ...
                  <input
                    type="text"
                    name="orderNumber"
                    required={isGoBikeOnline}
                    value={formData.orderNumber}
                    onChange={handleInputChange}
                    className="w-full px-5 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                    placeholder={isGoBikeOnline ? 'e.g. #12345 (required)' : 'e.g. #12345 (if available)'}
                  />
                </div>

                {/* ── Address Form: শুধু Non-GoBike shop এর জন্য ── */}
                {!isGoBikeOnline && (
                  <div className="border border-blue-100 bg-blue-50/40 rounded-2xl p-6 space-y-4">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-blue-600 text-lg">📦</span>
                      <h3 className="font-bold text-gray-900 text-[15px]">Shipping Address</h3>
                      <span className="text-[12px] text-gray-500">(where to send replacement parts)</span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          type="text" name="firstName" required={!isGoBikeOnline}
                          value={addressForm.firstName} onChange={handleAddressChange}
                      ...
                      <div>
                        ...
                        <input
                          type="text" name="lastName" required={!isGoBikeOnline}
                          value={addressForm.lastName} onChange={handleAddressChange}
                          className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                          placeholder="Doe"
                        />
                      </div>
                    ...
                    <div>
                      ...
                      <input
                        type="text" name="address1" required={!isGoBikeOnline}
                        value={addressForm.address1} onChange={handleAddressChange}
                        className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                        placeholder="e.g. 52 Bligh Avenue"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={addressForm.address2} onChange={handleAddressChange}
                        className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1.5">Suburb / City *</label>
                        <input
                          type="text" name="city" required={!isGoBikeOnline}
                      ...
                      <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1.5">Postcode *</label>
                        <input
                          type="text" name="postcode" required={!isGoBikeOnline}
                          value={addressForm.postcode} onChange={handleAddressChange}
                          className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1.5">State / Territory *</label>
                        <select
                          name="state"
                          value={addressForm.state}
                          onChange={handleAddressChange}
                          className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none cursor-pointer transition-all"
                        >
                          {AU_STATES.map((s) => (
                    ...
                    <div>
                      ...
                      <input
                        type="tel" name="phone" required={!isGoBikeOnline}
                ...
                <div>
                  ...
                  {uploadedMedia.length > 0 && !isUploading && (
                    <div className="mt-4 p-4 border border-green-200 bg-green-50 rounded-xl">
                      <p className="text-[13px] font-bold text-green-700 mb-3">
                        ✓ {uploadedMedia.length} File(s) Ready to Submit:
                      </p>
                      <ul className="space-y-2">
                        {uploadedMedia.map((media, idx) => (
                          <li key={idx} className="flex justify-between items-center bg-white p-3 rounded-lg border border-green-100 text-sm shadow-sm">
                            <a href={media.url} target="_blank" className="text-blue-600 hover:underline font-medium truncate max-w-[70%]">
                              {media.filename} - View
                            </a>
                            <button
                              type="button" onClick={() => removeMedia(idx)}
                              className="text-red-500 hover:text-white hover:bg-red-500 font-bold px-3 py-1 rounded transition-colors text-xs"
                            >
                              Remove
                            </button>
                          </li>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none resize-none transition-all"
                  />
                </div>

                <button
                  type="submit"
                  disabled={isSubmitting || isUploading || uploadedMedia.length === 0}
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl disabled:opacity-70 disabled:cursor-not-allowed shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-0.5"
                >
